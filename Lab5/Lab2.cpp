...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        myeditor->Start(hWnd, new PointShape);
        mytable->OnCreate(hWnd, hInst);
        break;
    ...
    case WM_LBUTTONUP: //відпущено ліву кнопку миші у клієнтській частині вікна
        mytable->Add(myeditor->OnLBup(hWnd));
        break;
    ...
    case WM_PAINT: //потрібно оновлення зображення клієнтської частині вікна
        myeditor->RemoveItem(mytable->getRemove() - 1);
        mytable->setRemove(0);
        myeditor->OnPaint(hWnd, mytable->getSelected() - 1);
        break;
    ...
    case WM_COMMAND:
    {
        ...
        case ID_32772:
            ...
            myeditor->Start(hWnd, new LineShape);
            break;

        case ID_TOOL_RECT:
        case ID_32773:
            myToolBar.OnTool(hWnd, ID_TOOL_RECT);
            myeditor->Start(hWnd, new RectShape);
            break;

        case ID_TOOL_ELLIPSE:
        case ID_32774:
            myToolBar.OnTool(hWnd, ID_TOOL_ELLIPSE);
            myeditor->Start(hWnd, new EllipseShape);
            break;

        case ID_TOOL_LINEOO:
        case ID_32775:
            myToolBar.OnTool(hWnd, ID_TOOL_LINEOO);
            myeditor->Start(hWnd, new LineOOShape);
            break;

        case ID_TOOL_CUBE:
        case ID_32776:
            myToolBar.OnTool(hWnd, ID_TOOL_CUBE);
            myeditor->Start(hWnd, new CubeShape);
            break;
