...
INT_PTR CALLBACK Gener(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IDOK)
        {
            ...
            if (xMin > xMax || yMin > yMax || nPoint < 2 || nPoint > 30 || abs(xMin) > 240 || abs(xMax) > 240 || abs(yMin) > 240 || abs(yMax) > 240 )
            {
                MessageBox(hDlg, L"Неправильно введені дані", L"Спробуйте ще", MB_OK | MB_ICONERROR);
                break;
            }
